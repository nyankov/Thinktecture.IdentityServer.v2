...
{
    public class ConfigurationDatabaseInitializer
    {


        public static void SeedContext(bool loadTestData = false)
        {
            ...
            if (loadTestData && entry != null)
            {
                ...
                if (bool.TryParse(entry, out createData))
                {
                    ...
                    {
                        // test configuration
                        configurationRepository.Global = CreateTestGlobalConfiguration();
                        configurationRepository.WSFederation = CreateTestWSFederationConfiguration();
                        configurationRepository.WSTrust = CreateTestWSTrustConfiguration();
                        configurationRepository.FederationMetadata = CreateTestFederationMetadataConfiguration();
                        configurationRepository.OAuth2 = CreateTestOAuth2Configuration();
                        configurationRepository.AdfsIntegration = CreateTestAdfsIntegrationConfiguration();
                        configurationRepository.SimpleHttp = CreateTestSimpleHttpConfiguration();
                        configurationRepository.Diagnostics = CreateTestDiagnosticsConfiguration();
                        configurationRepository.OpenIdConnect = CreateTestOpenIdConnectConfiguration();
            ...
            if (configurationRepository.Global == null)
                configurationRepository.Global = CreateDefaultGlobalConfiguration();
            if (configurationRepository.WSFederation == null)
                configurationRepository.WSFederation = CreateDefaultWSFederationConfiguration();
            if (configurationRepository.WSTrust == null)
                configurationRepository.WSTrust = CreateDefaultWSTrustConfiguration();
            if (configurationRepository.FederationMetadata == null)
                configurationRepository.FederationMetadata = CreateDefaultFederationMetadataConfiguration();
            if (configurationRepository.OAuth2 == null)
                configurationRepository.OAuth2 = CreateDefaultOAuth2Configuration();
            if (configurationRepository.AdfsIntegration == null)
                configurationRepository.AdfsIntegration = CreateDefaultAdfsIntegrationConfiguration();
            if (configurationRepository.SimpleHttp == null)
                configurationRepository.SimpleHttp = CreateDefaultSimpleHttpConfiguration();
            if (configurationRepository.Diagnostics == null)
                configurationRepository.Diagnostics = CreateDefaultDiagnosticsConfiguration();
            if (configurationRepository.OpenIdConnect == null)
                configurationRepository.OpenIdConnect = CreateOpenIdConnectConfiguration();
        }
